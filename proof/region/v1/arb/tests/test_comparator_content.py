...
две координаты — поменять местами, и весь набор оставался зелёным. Значение
координаты переопределялось молча, хотя идентичность компаратора — фундамент
решающей цепи.
...
3. Структурный golden: разбор преимиджа на (сепаратор, версия, упорядоченные
   ...
   сепаратора и любой сдвиг схемы дают читаемый diff; правка interval.c —
   не даёт. Golden поэтому не подлежит регенерации «ради зелёного CI»:
   его движение всегда семантическое.

Байты допущенных исходников закреплены отдельно — ``_PINNED_BUILD_SOURCE_SHA256_V1``
в ``arb/pipeline.py``. Слои не дублируют друг друга: там — содержимое файлов,
здесь — смысл координат.
...
процессов сборки, которая закреплена своим golden в ``tests/test_build.py``.
Структурный golden покрывает их полностью — процессы в нём непрозрачны.
"""
    ...
    def test_decoder_reproduces_every_derived_coordinate_byte_for_byte(self) -> None:
        ...
        for field in dataclass_fields(preimages):
            with self.subTest(coordinate=field.name):
                ...
                self.assertEqual(replayed, preimage)
