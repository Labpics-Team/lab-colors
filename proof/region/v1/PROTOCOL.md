...
Протокол переносит immutable job и заявленные результаты Arb/MPFI
processes. Python controller имеет только structural authority:
безопасный process/file IO, строгий parse → typed value → byte-identical
re-encode, replay content identities, content-store resolution и сравнение
структурно admitted artifacts. Controller не вычисляет цвет или interval
enclosure, не классифицирует point и не создаёт semantic proof type.
...
Вычислители разделяют только immutable `ProofJobV1` bytes и этот protocol; их
formula, region и evaluator implementations не имеют общего кода с controller
или друг с другом.
...
| `RunReceiptV1` | `LCRUN1\0\0` | `labcolors.proof-region.run-receipt.v1` |
...
Trim, изменение line endings, normalisation или подмена evaluator source hash
запрещены.
...
definition. Job является единственным входом обоих вычислителей;
альтернативный JSON/TOML definition запрещён.
...
после того, как trusted `resolve_content_address` для каждой из десяти
...
Content resolution доказывает только наличие и exact identity заявленных
artifacts. Оно не доказывает корректность evaluator, build или test receipt и
не создаёт semantic source-bound/proven type.

V1 не вычисляет отдельную агрегированную independence coordinate.
...
`RunReceiptV1.binary_identity`. Два build одного engine не являются
независимыми. Общая явно связанная GMP/MPFR arithmetic closure разрешена.
...
Wire parse создаёт только structural run claim. При controller construction
binary, platform и invocation identities вычисляются из фактически
наблюдаемого run и связываются с transcript того же job и comparator.
Ни raw parse, ни эта связь не доказывают evaluator semantics.
...
7. Arb и MPFI run receipt identities;
...
`ContentResolvedComparatorManifestV1`, согласованных `RunReceiptV1` и
...
неподтверждённая independence или structurally missing equality witness возвращают typed
...
`invalid_transcript`, `missing_equality_witness`, `foreign_binding`, `not_independent`,
