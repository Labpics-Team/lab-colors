...
//! получает те же CAM16/LCS-координаты, замороженную Ys candidate-кривую и
//! нормативные WCAG-выходы, что и WASM-граница — из одного Rust-ядра.
//!
...
use labcolors_core::{
    BgInput, ChromaPolicy, Contract, Gamut, Hue, NumericalDecisionEvidenceV1, ViewingConditions,
...
impl Theme {
    fn vc(self) -> ViewingConditions {
        match self {
            Theme::Light => ViewingConditions::srgb(),
            Theme::Dark => ViewingConditions::dim_surround(),
            Theme::LightIc => ViewingConditions::srgb_high_contrast(),
            Theme::DarkIc => ViewingConditions::dim_surround_high_contrast(),
        }
    }
}
...
/// Две независимые координаты переднего плана на фоне: знаковая кандидатная
/// оценка по `Ys` и юридический WCAG-ratio.
#[derive(Debug, Clone, Copy, PartialEq, uniffi::Record)]
pub struct Contrast {
    /// Знаковая кандидатная оценка `Lc` по `Ys` из замороженной SAPC-shaped
    /// кривой; не LPC/readability evidence.
    pub lc: f64,
...
/// Резолвнутый цвет и его численные координаты на declared background.
...
/// Две координаты одного переднего плана на фоне — `(Ys candidate score, WCAG)`.
/// Дешёвый runtime-примитив декодирует каждый sRGB8-вход в display relative
/// luminance и применяет замороженную кривую отдельно от WCAG.
///
/// # Errors
///
...
/// Перепроверка двух численных координат многих передних планов на одном фоне.
/// Display relative luminance фона вычисляется один раз на батч. Сам вызов не
/// выносит LPC/readability verdict: критерий применяет вышестоящий evaluator.
///
/// # Errors
///
