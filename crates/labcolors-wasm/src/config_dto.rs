...
/// "labui"` (без `roles`) и полный SSOT-паспорт наполняют один и тот же
/// словарь, поэтому ОБЯЗАНЫ дать один отпечаток. `canonical_dto` (приватный
/// хелпер ниже — потому не линк) раскрывает
...
mod tests {
    ...
    /// плюс `preset`. Строим из замороженного SSOT-паспорта (те же якоря/ручки),
    /// обнуляя roles/aliases (дерево Даниила ушло из прод-API ядра, ADR-0001 PR-c).
    fn thin_labui_dto() -> ConfigDto {
        let mut dto = labui_dto();
        dto.roles = Vec::new();
        dto.aliases = Vec::new();
        dto.preset = Some(RolePresetDto::Labui);
        dto
    }
    ...
    /// ОДИН отпечаток. Оба считаются ЖИВЬЁМ из замороженного SSOT-паспорта, поэтому тест
