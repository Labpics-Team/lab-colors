...
///
/// Не криптографический и не обязан им быть: назначение — ключ контракт-кэша
/// внутри процесса (разные конфиги → разные пространства ключей). Детерминизм
/// даёт serde: порядок полей структур фиксирован, вход нормализуется парсингом.
