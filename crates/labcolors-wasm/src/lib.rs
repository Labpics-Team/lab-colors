...
///
/// Владеет ровно одной Session; update атомарен, DOM/CSS-side effects
/// принадлежат npm-адаптеру, не WASM.
#[wasm_bindgen]
...
#[wasm_bindgen]
impl ProgramRuntime {
    /// Атомарный observed update. `scenario_ids.len()` определяет число
    /// сценариев; `surfaces` — плоская матрица row-major, каждый row имеет
    /// `surface_count × 3` bytes. Некратная длина — typed invalid_config.
