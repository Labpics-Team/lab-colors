...
fn parse_public_api(path: &Path) -> Vec<ExportItem> {
    ...
    while i < lines.len() {
        ...
        let has_gate = feature_gate.is_some();
        let (start_line_idx, gate) = if has_gate && i + 1 < lines.len() {
            (i + 1, feature_gate.clone())
        } else {
            (i, None)
        };

        let start_trimmed = lines[start_line_idx].trim();

        // Multi-line `pub use` accumulation: collect until we find the closing `;`.
        // This handles grouped imports spanning multiple lines like:
        //   pub use solve::{
        //       BgInput, ChromaPolicy, ...,
        //       solve, solve_many,
        //   };
        if start_trimmed.starts_with("pub use ") && !start_trimmed.ends_with(';') {
            let mut accumulated = String::from(start_trimmed);
            ...
            for item in parse_pub_line(&accumulated, gate.clone()) {
                out.push(item);
            }
            i = j + 1;
            continue;
        ...
        for item in parse_pub_line(start_trimmed, gate.clone()) {
            out.push(item);
        }
...
fn parse_pub_line(line: &str, feature_gate: Option<String>) -> Vec<ExportItem> {
...
fn parse_pub_use_items(use_rest: &str) -> Vec<ExportItem> {
    ...
    if let Some(brace_start) = cleaned.find('{') {
        ...
        for part in inner.split(',') {
            ...
            let source = part.split(" as ").next().unwrap_or(part).trim();
            out.push(ExportItem::ReExport {
                name,
                source_path: format!("{}::{}", prefix, source),
            });
        }
        return out;
    }
