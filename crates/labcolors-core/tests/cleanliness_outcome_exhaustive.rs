...
//! * unit-тесты сравнивают `ALL` с рукописным `DECLARATION_ORDER`, то есть с
//!   ещё одним рукописным списком: автор, забывший `ALL`, забудет и его.
//!
...
fn the_scanner_actually_reads_the_source() {
    ...
    // Удалённый вариант не вправе вернуться незамеченным: его отсутствие —
    // решение раунда 6 ревью контракта, а не случайность.
    assert!(
        !OUTCOME_SOURCE.contains("UnchangedBelowOutputResolution"),
        "исход снят контрактом: суб-LSB допуск ловится при сборке, а не в рантайме"
    );
}
