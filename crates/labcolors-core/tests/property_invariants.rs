...
// ─────────────────────────────────────────────────────────────────────────────
// СВОЙСТВО 2 — ratio-пол держится на КВАНТОВАННЫХ байтах (полярность/пол)
//
// КЛАСС: «движок эмитит цвет текст/UI-роли, нарушающий свой ratio-пол на
        ...
        |(r, g, b, vc_i)| {
            ...
            for (name, spec) in table.entries() {
                let Some(floor) = spec.floor_ratio() else {
                    continue; // роль без ratio-пола — пол не применяется
                };
