...
/// Каждый `*.rs` дерева модуля: `cleanliness.rs` плюс всё под `cleanliness/`
/// **на любой глубине**.
///
/// Обход рекурсивен. Плоский обход пропускал файл в каталоге подмодуля
/// (`cleanliness/registry/scale.rs`) — обычная раскладка Rust, уже
/// встречающаяся в этом крейте, а не трюк. Оба сужения — жёсткий список файлов
/// и плоский каталог — найдены ревью как рабочие обходы, а не предположены.
fn module_sources() -> Vec<(String, String)> {
    ...
    walk(&root.join("cleanliness"), &mut files);
...
fn the_field_scanner_actually_reads_the_source() {
    ...
    // Вторая половина сканера: без этих ассертов её можно было бы выпотрошить
    // до пустоты, и сюита осталась бы зелёной — ни один поставляемый файл
    // модуля строк уровня элемента не содержит.
    let items = item_level_type_lines("pub type S = u8;\ncarrier!(S2, u8);\n");
    assert_eq!(
        items.len(),
        2,
        "псевдоним и макро-вызов обязаны попадать в разбор"
    );
    assert!(
        item_level_type_lines("    assert!(x == 1);\n").is_empty(),
        "макрос внутри блока строкой уровня элемента не является"
    );
    assert!(
        item_level_type_lines("//! * макро-вызов уровня элемента, содержащий число;\n").is_empty(),
        "строка документации разбором уровня элемента не считается"
    );
