...
потребителей Core. Транспорты Protocol/WASM/FFI в V1 проецируют только
зарегистрированную нейтральную ось. В этом релизе явный клиентский набор
является Core-only API: Protocol/WASM/FFI его не проецируют. Эта граница не
создаёт второй Cargo-feature и не меняет математику общего компилятора.
