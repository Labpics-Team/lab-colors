...
pub enum RoleSpec {
    ...
    /// foreground occurrence против неё. Доказательный статус downstream-
    /// резолвера граф не назначает. Differential-тест закрепляет эквивалентность
    /// миграционного wiring и outcomes на проверяемом домене, но не является
    /// независимым oracle самого solver-а. Тон клампится (флаг `compressed`)
    /// при недостижимости на кривой семьи.
    PairLabel {
...
/// Валидация домена — потому что RoleSpec публичен (см. `rgba_input_valid`).
...
/// граф не присваивает этому downstream-решению доказательный статус: он
/// возвращает только source/against/backdrop. Differential закрепляет wiring и
/// outcomes миграции на проверяемом домене, но оба пути используют один solver
/// и потому не образуют независимый oracle его математики. Собственный
/// [`ResolveContext`] поверхности задаёт полярность/макс-контраст, поэтому
/// WCAG-пол лейбла
