...
/// палитры, ровно как фон — вход движка, а не роль. Семейства перечислены по
/// возрастанию Oklab-оттенка (красный → пурпурный/розовый), чтобы порядок
/// читался как обход цветового круга.
...
impl Accent {
    /// Все 10 семейств палитры в порядке обхода круга (Red → Pink) — поверхность
    /// для property-свипов в тестах и для будущей роль-таблицы акцентов (#59).
