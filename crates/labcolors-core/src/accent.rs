...
//! каждое несёт единственный измеренный якорный hex (`anchor_hex`),
//! из которого выводится якорный Oklab-оттенок (`prototype_hue`)
//! и полная рампа через `curve` (та же `AccentCurve`, что
...
//! `reference/labui-accent-primitives.md`. Якорный hex — это
...
//! `AccentCurve::at`). Поэтому тёмный/IC-вариант
