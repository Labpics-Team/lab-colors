...
//! `oklch(L% C H / A)` — один парсер, одна форма, явно именованные
//! компоненты.
//!
...
//! sRGB8. Исполняемый тест проверяет детерминированную решётку с шагом 5 по
//! каждому каналу (около 140 тысяч цветов, включая края) и весь серый ramp.
//! Это сильная выборочная проверка, но не полный перебор 16 777 216 входов;
//! переносимость за пределы проверенных target/toolchain она не объявляет.
...
/// опциональной альфы.
///
/// Точность: L — 5 знаков процента, C — 6, H — 3. Байт-точность проверяется
/// исполняемой решёткой с шагом 5 и полным серым ramp. Полный sRGB8-куб пока не
/// заявляется доказанным: выборочный тест нельзя переименовывать в exhaustive.
