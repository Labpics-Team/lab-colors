...
/// Право не несёт состояния сверх свидетельства.
///
/// Область проверки узкая, и её надо назвать честно: тест краснеет, если к
/// праву добавят размерное поле, но **необитаемости не доказывает** — у
/// обитаемой unit-структуры размер тоже нулевой. Необитаемость сторожит
/// `absurd()`: его `match self {}` перестанет компилироваться, как только у
/// носителя появится вариант.
#[test]
