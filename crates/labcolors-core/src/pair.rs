...
/// интервала даёт 0.305, 0.30 — округление внутри интервала. Терминал закрыт
/// декларацией: психофизический эксперимент не требуется и путём разрешения не
/// является (решение владельца 2026-07-07 — таксономия (a)/(b)/(e)).
// SSOT-TRACKED — Y-порог кроссовера стороны пары, терминал (e) design-choice, см. docs/empirical-inventory.md.
