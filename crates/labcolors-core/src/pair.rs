...
/// [`pair_side`] сравнивает ЧИСТЫЙ WCAG-люминанс якоря (Rec.709, `wcag_y_encoded`
/// — без хромы, без CAM16) с этим порогом: ниже — светлый лейбл, выше —
/// чернильный. Значит порог обязан стоять ровно там, где обе стороны РАВНО
/// читаемы, — в том же чисто-люминансном домене, в котором принимается решение.
///
...
// ─────────────────────────────────────────────────────────────────────────────
// Научные локи терминала (a) DERIVED для PAIR_CROSSOVER_Y (после #180: (e)→(a),
// значение = измеренный перелом ядра `contrast_core`). Значение НЕ меняется —
// тесты предъявляют: (1) классификационный выход (сторона пары) на реальной
// палитре labui НЕ зависит от точного порога внутри задекларированного интервала
// (робастность деривации); (2) константа РАВНА измеренному перелому полярности
// ядра (деривация, а не цитата); (3) доля гаммы в зоне флипа замерена.
// ─────────────────────────────────────────────────────────────────────────────
#[cfg(test)]
mod exposure_locks {
    ...
    ///    бисекция. Это ДОМЕН РЕШЕНИЯ `pair_side` (сравнивает чистый WCAG-люминанс),
    ///    потому его перелом = якорь `PAIR_CROSSOVER_Y`.
    ...
    fn pair_crossover_equals_measured_core_polarity_flip() {
        ...
        // Снапшот замера ядра (регрессионный якорь; прежний rustdoc-клейм
        // «перелом ≈ 0.36» опровергнут этим замером 2026-07-07).
        assert!(
        ...
        // (a) DERIVED: константа РАВНА измеренному перелому ядра — домен решения
        // `pair_side` чисто-люминансный, потому якорь = ядро `contrast_core`, а не
        // полная метрика (byte 155, Y≈0.325 несёт CAM16/Y_hk-реконструкцию, чужую
        // решению; она остаётся вторичным замером, не якорем константы).
        assert!(
