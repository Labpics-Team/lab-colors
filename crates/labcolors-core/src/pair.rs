...
mod exposure_locks {
    ...
    /// Инвариант: 12 якорей 49-якорного паспорта лежат в зоне
    /// (тёмные/IC-варианты семей, напр. info dark #5696FF
    ...
    fn exposure_pair_crossover() {
        ...
        assert_eq!(
            labui_hits.len(),
            12,
            "в зоне перелома должны оставаться ровно 12 из 49 якорей"
        );
