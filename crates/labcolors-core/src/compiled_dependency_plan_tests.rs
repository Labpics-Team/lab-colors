...
fn selective_update_matches_full_resolve_on_diamond_graph() {
    let plan = CompiledDependencyPlanV1::compile(
        &[0, 1, 2, 3],
        &[(0, 1), (0, 2), (1, 3), (2, 3)],
    )
    ...
    assert!(affected_raw.contains(&3), "changed node itself must be in affected set");
    assert!(affected_raw.contains(&1), "direct dependent 1 must be affected");
    assert!(affected_raw.contains(&2), "direct dependent 2 must be affected");
    assert!(affected_raw.contains(&0), "transitive dependent 0 must be affected");
    assert_eq!(affected_raw.len(), 4, "all nodes must be affected in diamond when root changes");
    ...
    assert!(!affected_1_raw.contains(&2), "sibling 2 must NOT be affected by change to 1");
    assert!(!affected_1_raw.contains(&3), "dependency 3 must NOT be affected by change to 1");
}
...
fn sabotage_missing_transitive_dependent_detected() {
    ...
    assert!(raw.contains(&2), "transitive dependent at depth 2 must be included");
    assert!(raw.contains(&3), "most distant transitive dependent must be included");
    assert_eq!(raw.len(), 4);
}
...
fn sabotage_semantic_name_branching_detected() {
    ...
    let affected = plan.affected_nodes(&[NodeIndexV1::new(0)]);
    assert_eq!(affected.len(), 3, "all nodes must be reachable via opaque indices");
}
...
fn sabotage_byte_equality_stale_reuse_detected() {
    ...
    assert!(a.len() > b.len(), "changing root must affect more nodes than changing leaf");
}
