...
fn operators_have_separate_identities_and_reference_results() {
    ...
    // V1-байты независимо закодированы struct.pack и проверены hashlib.sha256.
    // Эти эталоны защищают старую каноническую идентичность при смене хранения.
    assert_eq!(
        request_digest(&source_over).as_bytes(),
        [
            0x5c, 0x26, 0xaa, 0x51, 0x5f, 0x2e, 0xb2, 0x30, 0xc6, 0xb1, 0xb, 0x98, 0x24, 0x15,
            0xb5, 0xeb, 0xc1, 0x89, 0xf9, 0xc3, 0x37, 0xb, 0x1e, 0x58, 0xec, 0xe9, 0x14, 0x43,
            0xd8, 0x3e, 0x3f, 0xc8
        ]
    );
    assert_eq!(
        request_digest(&screen).as_bytes(),
        [
            0xb6, 0x6, 0x26, 0xe3, 0x6, 0x5e, 0xd0, 0x5f, 0xdf, 0x88, 0x49, 0x52, 0x7c, 0x76, 0x8e,
            0xfa, 0x1f, 0x93, 0x3c, 0xdf, 0x44, 0xb9, 0x8c, 0xc3, 0x47, 0x78, 0xa4, 0x17, 0xcf,
            0x64, 0x3e, 0x2f
        ]
    );
    assert_eq!(
        request_digest(&lighter).as_bytes(),
        [
            0xf3, 0x32, 0xa7, 0xa0, 0x12, 0x1a, 0xe, 0x79, 0x1, 0x2d, 0xbe, 0x44, 0xc2, 0x67, 0x15,
            0x24, 0x1a, 0x37, 0xd6, 0xdf, 0x24, 0xae, 0x13, 0xb6, 0x7f, 0xcc, 0x29, 0xc1, 0xde,
            0xa5, 0x6e, 0xb2
        ]
    );
