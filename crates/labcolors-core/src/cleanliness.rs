...
// Форма dΔH_BB/dh = A·cos(h−h_Y) — первогармоническое (fundamental Fourier)
// приближение периодической BB-кривой (зануляется на уникальных оттенках, максимум
// между ними); из табличных данных статьи напрямую не выводится — приближение,
// принятое движком.
...
///
/// Вывод (в том виде, в каком получена замороженная константа):
...
///
/// ⚠️ ВНИМАНИЕ (переверифицировано colour-science 0.4.7, CIE 1931 2° Standard
/// Observer): узлы CMF в шаге 1 РАСХОДЯТСЯ с официальной таблицей CIE. Фактически
/// 570nm x̄=0.7621 (задекларированное 0.8425 — это значение 575nm, сдвиг на один
...
/// # Вывод формулы
///
/// Эффект Бецольда-Брюкке (Jacobs & Wascher 1967, «Bezold–Brücke Hue Shift: Further
/// Measurements», J. Opt. Soc. Am. 57(9):1155, DOI 10.1364/josa.57.001155): при росте
/// яркости оттенки смещаются к уникальному жёлтому (h_Y) и уникальному синему.  Производная
/// смещения по Oklab-оттенку:
///
...
///
/// Форма `A·cos(h−h_Y)` — первогармоническое (fundamental Fourier) приближение
/// периодической BB-кривой; из табличных измерений Jacobs & Wascher (1967) напрямую
/// не выводится, а принята движком как гладкое периодическое ядро, зануляющееся на
/// уникальных оттенках и максимальное между ними.
///
/// Нормируем в [0, 1]:
///
...
///   Цитата: Jacobs & Wascher (1967) J. Opt. Soc. Am. 57(9):1155 (DOI 10.1364/josa.57.001155);
///   König & Dieterici (1884) unique-hue loci.
///
...
///
/// Свойства:
///   - Выводных констант: только H_Y_DEG = 96.9° (CIE D65 unique yellow, Jacobs & Wascher 1967;
///     ⚠️ узлы CMF деривации расходятся с CIE 1931 2° — см. флаг у `H_Y_DEG`)
...
mod tests {
    ...
    //   ⚠️ Узлы CMF расходятся с официальной CIE 1931 2° (см. флаг у `H_Y_DEG`); значение
    //   заморожено и в этом PR не меняется.
    //   Формула (1 + cos(h − h_Y)) / 2 — Hanning-окно, первогармоническое приближение
    //   производной поворота Бецольда-Брюкке dΔH_BB/dh = A_BB · cos(h − h_Y), нормированное
    //   в [0, 1].  Цитата: Jacobs & Wascher (1967) J. Opt. Soc. Am. 57(9):1155 (DOI
    //   10.1364/josa.57.001155).
