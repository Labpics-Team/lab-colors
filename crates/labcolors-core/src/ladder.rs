#![allow(dead_code)] // V1 ladder positions staged for labui role generation.
...
//! Labui-контракт эмитит пару (тинт, α) НАПРЯМУЮ (композитит браузер), а НЕ
//! солид-эквивалент. Поэтому позиция лестницы = `{тинт = якорь источника по теме,
//! α из меню}`; резолв несёт полупрозрачный исход
//! (см. [`crate::semantic::Resolved::Translucent`]).
