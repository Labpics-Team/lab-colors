...
//!
//! core — малый радиус, светлота поднята к белому, хрома понижена
//! (переэкспонирование центра — сигнатура реального света); halo — большой
//! радиус, полная хрома источника. Оттенок оба слоя наследуют от источника:
//! свечение не имеет собственного цвета. Вывод core: полпути к белому по J'
//! и половина колоритности M' (фотографическая аналогия «+1 стоп в пересвет»;
//! DECLARED-деривация, зафиксирована тестом).
...
///
/// halo — сам источник (полная хрома); core — переэкспонирование: полпути к
/// белому по J' (`J'core = (J'src + 100)/2`) при половине колоритности M'
/// (пересвет обесцвечивает центр). Оттенок (h_ok, h_cam) — источника:
/// свечение не имеет собственного цвета.
pub fn glow_layers_from_source(
    ...
    // s = M'/(J'+1); половина M' при новом J':
    let mp_core = src.s * (src.jp + 1.0) * 0.5;
    let s_core = mp_core / (jp_core + 1.0);
    let core = LcsColor::new(jp_core, src.h_ok, s_core, src.h_cam());
...
mod tests {
    ...
    /// Анатомия core: светлее источника и менее колоритен (пересвет),
    /// оттенок унаследован.
    #[test]
    fn core_layer_is_overexposed_source() {
        let vc = ViewingConditions::dim_surround();
        ...
        assert!(core.jp > src.jp, "core светлее источника");
        let mp_src = src.s * (src.jp + 1.0);
        let mp_core = core.s * (core.jp + 1.0);
        assert!(mp_core < mp_src, "core менее колоритен (пересвет)");
