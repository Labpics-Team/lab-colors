//! Замороженная совместимая модель «Закон Грязи Labpics V1».
//!
//! Исторические уравнения и результаты сохранены точно, чтобы native/WASM/FFI
//! потребители и conformance-векторы не меняли смысл молча. V1 —
//! охарактеризованная гипотеза Labpics, а не валидированная вероятность для
//! популяции и не универсальное психофизическое определение чистого/грязного.
//! Новый код должен использовать явно версионированный отчёт V2 из родительского
//! модуля `cleanliness`.
//!
//! # Уравнения
//!
...
/// Исторический sRGB → Oklab-конвейер, на котором были рассчитаны V1-векторы.
///
/// V1 обязан замораживать не только собственную формулу, но и её координатную
/// реализацию. Вызов живого `spaces::oklab` незаметно изменил бы сотни старых
/// отметок при каждом уточнении матриц, аналитической ахроматической ветви или
/// порядка операций. Коэффициенты и порядок ниже дословно повторяют прежний
/// binary64-путь lab-colors; новый код вне V1 использовать его не должен.
fn legacy_srgb_linear_to_oklab([r, g, b]: [f64; 3]) -> [f64; 3] {
...
pub fn muddiness_from_linear_srgb(rgb: [f64; 3]) -> f64 {
    let lab = legacy_srgb_linear_to_oklab(rgb);
    let l = lab[0];
