...
impl Hasher {
    ...
    pub(crate) fn update(&mut self, mut bytes: &[u8]) {
        ...
        if self.pending_len != 0 {
            let copied = bytes.len().min(64 - self.pending_len);
    ...
    pub(crate) fn finalize(mut self) -> Digest {
        ...
        if self.pending_len >= 56 {
            final_block[self.pending_len + 1..].fill(0);
            compress(&mut self.state, &final_block);
            final_block = [0; 64];
        } else {
            final_block[self.pending_len + 1..56].fill(0);
        }
        ...
        for (word, bytes) in self.state.into_iter().zip(output.chunks_exact_mut(4)) {
