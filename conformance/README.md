...
- **Версия пака** (`manifest.packVersion`, сейчас `10.0.0`) — семантическая
  версия СХЕМЫ и состава векторов. Bump 9.0.0 → 10.0.0 удалил ровно одно
  семейство `muddiness`: legacy cleanliness-прокси вырезан из всех проекций
  (Oklab-преобразования, гамут-границы и агностичность живут в своих модулях),
  байты остальных пяти семейств не тронуты.
  Предыдущий bump 8.0.0 → 9.0.0 удалил ровно одно
