...
### Наблюдение фона (внутренняя граница)

Package-private helpers разбирают поддерживаемую цепочку CSS-цветов для
runtime-контроллеров; они не являются функцией публичного root API и не
доказывают, что пользователь видит именно этот пиксель.
...
Результат внутренней observation boundary нельзя использовать как сертификат
