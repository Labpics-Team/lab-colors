...
Лестницы `fill-*` и `border-base`/`border-soft` несут буквальные dJ'-якоря — перцептивный шаг светлоты `J'` относительно поверхности, измеренный из Figma-структуры LabUI отдельно под каждую тему. Это различимость, а не разборчивость текста, поэтому пола читаемости у них нет. `border-strong` несёт силу `label-primary` (доля 0.968·max), но с non-text-полом 3:1 (WCAG 1.4.11): граница должна быть различима, не читаема. Стек теней и `separator` держат Lc-величины выше надёжного пола решателя (`DECORATIVE_FLOOR_MIN` ≈ 7.5 Lc); их финальная JND-калибровка по альфа-якорям — глава `surface-jnd`. `border-ghost`, `fill-none` и `none` — явный ноль (`Resolved::None`).
