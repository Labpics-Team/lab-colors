...
**Выход:** цветовая система целиком — таблица именованных ролей, темы (light/dim), контраст с юридическим полом WCAG 2.1 AA.
...
### Конформанс: WCAG2-floor + LPC-target

LPC — перцептивная цель; юридический минимум — WCAG 2.1 AA (через [EN 301 549](https://www.etsi.org/deliver/etsi_en/301500_301599/301549/03.02.01_60/en_301549v030201p.pdf) его требует European Accessibility Act): 4.5:1 для текста, 3:1 для UI-элементов. Решатель держит обе планки: целится в LPC-таргет, затем проверяет стандартный relative-luminance ratio выходного цвета; если закон строже перцепции — цвет досдвигается до пола и результат помечается флагом `floor_override`. Оба числа (`lc` и `wcag_ratio`) отчитываются раздельно и никогда не смешиваются. Декоративные контракты (сепараторы, тени — `Contract::range`) юридического пола не получают: к ним требования читаемости не применяются.
...
|------|------|-----------|--------|----------|
| `LabelPrimary` | `label-primary` | лейбл (текст) | `Anchor` 0.97335917·max | AA текст 4.5:1 |
| `LabelSecondary` | `label-secondary` | лейбл | `Anchor` 0.64359014·max | AA текст 4.5:1 |
| `LabelTertiary` | `label-tertiary` | лейбл | `Anchor` 0.47572199·max | AA UI 3:1 |
| `LabelQuaternary` | `label-quaternary` | лейбл | `Anchor` 0.29335999·max | нет (WCAG исключает inactive) |
| `Icon` | `icon` | UI | `Anchor` 0.47572199·max | AA UI 3:1 |
| `Separator` | `separator` | декор (Lc) | `Decorative` 8.0 | нет |
| `BorderStrong` | `border-strong` | граница | `Anchor` 0.97335917·max | AA UI 3:1 |
...
Лестницы `fill-*` и `border-base`/`border-soft` несут буквальные dJ'-якоря — перцептивный шаг светлоты `J'` относительно поверхности, измеренный из Figma-структуры LabUI отдельно под каждую тему. Это различимость, а не разборчивость текста, поэтому пола читаемости у них нет. `border-strong` несёт силу `label-primary` (доля 0.97335917·max), но с non-text-полом 3:1 (WCAG 1.4.11): граница должна быть различима, не читаема. Стек теней и `separator` держат Lc-величины выше надёжного пола решателя (`DECORATIVE_FLOOR_MIN` ≈ 7.5 Lc); их финальная калибровка по JND (just-noticeable difference — минимально заметная глазу разница) от альфа-якорей — глава `surface-jnd`. `border-ghost`, `fill-none` и `none` — явный ноль (`Resolved::None`).
