#!/usr/bin/env python3
"""EXT-08 sabotage controls and RED→GREEN proof tests.

Verifies that the decisions extractor:
1. Produces a valid manifest with correct schema.
2. Fails when manifest_sha256 is tampered (integrity sabotage).
3. Fails when decision_count mismatches actual decisions (consistency sabotage).
...
EXTRACTOR = REPO_ROOT / "scripts" / "extract_decisions.py"
...
class EXT08DecisionsTests(unittest.TestCase):
    """RED→GREEN proof + sabotage controls for EXT-08."""
    ...
    def test_extract_produces_valid_manifest_schema(self):
        ...
        self.assertEqual(manifest["class"], "decisions")
        self.assertEqual(manifest["schema_version"], 1)
        self.assertIn("decisions", manifest)
        self.assertIn("manifest_sha256", manifest)
        self.assertGreater(manifest["decision_count"], 0)
        for dec in manifest["decisions"]:
            self.assertIn("path", dec)
            self.assertIn("line", dec)
            self.assertIn("kind", dec)
            self.assertIn("expression", dec)
            self.assertIn("source_sha256", dec)
            self.assertIn(dec["kind"], ("match", "if-else", "enum-dispatch"))
    ...
    def test_verify_passes_on_current_tree(self):
        ...
        self.assertIn(b"EXT-08 VERIFY OK", verify.stderr)
    ...
    def test_sabotage_decision_count_mismatch_detected(self):
        """RED: verify fails when decision_count doesn't match actual decisions."""
        extract = run_extractor("extract")
        manifest = json.loads(extract.stdout)
        manifest["decision_count"] = 999999
        ...
        self.assertIn(b"decision_count mismatch", verify.stderr)
