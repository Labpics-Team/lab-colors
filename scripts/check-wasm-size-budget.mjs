...
const V14_PATH = resolve(REPO_ROOT, "packages/colors/bench/wasm-size-budget-v14.json");
const V15_PATH = resolve(REPO_ROOT, "packages/colors/bench/wasm-size-budget-v15.json");

export const DEFAULT_BUDGET = resolve(
  REPO_ROOT,
  "packages/colors/bench/wasm-size-budget-v16.json",
);
...
// ратчета); константа текущей версии (сейчас v16) ротируется ТОЛЬКО вместе с
...
export const V15_FILE_SHA256 =
  "979dc7990742cc2daf163c64051ed6a22c939745a4b1767bc010d6faaaba5161";
export const V16_FILE_SHA256 =
  "9a9ab7050a3f6b103905a817b9d675650d04ddab3524a3fc5aa3d0f5d6cab9c3";

const V1_REPOSITORY_PATH = "packages/colors/bench/wasm-size-budget-v1.json";
const V15_REPOSITORY_PATH = "packages/colors/bench/wasm-size-budget-v15.json";
...
const V15_BUDGET_ID = "labcolors-wasm-runtime-c6-validation-srgb-ssot-v15";
...
const ROLE_SPECS = {
  runtime: {
    ...
    // Pinned Linux run 29660556196 измерил WASM-артефакт Q0-среза ровно. 3709B —
    // наблюдаемая дельта артефакта к v15, не причинная оценка отдельных частей.
    // Любой дальнейший рост требует новой версии, не headroom здесь.
    basis: "accepted-q0-output-conflict-snapshot",
    measurementSource: "github-actions-run-29660556196",
    acceptedCeiling: 432112,
  },
};
...
function verifyImmutableHistory() {
  ...
  const v14 = readImmutableJson(V14_PATH, V14_FILE_SHA256, "v14");
  if (v14?.schemaVersion !== 8 || v14?.budgetId !== V14_BUDGET_ID) {
    fail("immutable v14 budget identity drifted");
  }
  const v15 = readImmutableJson(V15_PATH, V15_FILE_SHA256, "v15");
  if (v15?.schemaVersion !== 8 || v15?.budgetId !== V15_BUDGET_ID) {
    fail("immutable v15 budget identity drifted");
  }
  return { v1, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15 };
}
...
function validateBudgetValue(budget) {
  ...
  if (budget.budgetId !== V16_BUDGET_ID) fail(`budgetId must be ${V16_BUDGET_ID}`);

  exactKeys(budget.predecessor, ["path", "fileSha256"], "predecessor");
  if (
    budget.predecessor.path !== V15_REPOSITORY_PATH ||
    budget.predecessor.fileSha256 !== V15_FILE_SHA256
  ) {
    fail("predecessor must bind the immutable v15 document");
  }
...
export function parseBudgetDocument(bytes, budgetPath) {
  ...
  if (resolve(budgetPath) === DEFAULT_BUDGET) {
    ...
    if (actualFileSha256 !== V16_FILE_SHA256) {
      fail(
        `current v16 file SHA-256 mismatch: ` +
          `expected=${V16_FILE_SHA256} actual=${actualFileSha256}`,
      );
    }
  }
  return budget;
