...
def valid_report():
    return {
        ...
        "verification_results": {
            "summary": {"total_harnesses": 4, "executed": 4, "successful": 4,
                        "failed": 0, "status": "completed"},
            "results": [
                {"harness_id": PREFIX + name, "status": "Success", "checks": [
...
class FormalReportTests(unittest.TestCase):
    ...
    def test_missing_duplicate_extra_or_foreign_harness_is_rejected(self):
        ...
        for changed in ([], results[:-1], results + results[:1], [results[0]] * 4):
    ...
    def test_mutant_requires_target_semantic_failure(self):
        ...
        validate_mutant(report, 1)
        with self.assertRaises(ValueError):
            validate_mutant(report, 0)
        with self.assertRaises(ValueError):
            validate_mutant(report, 2)
