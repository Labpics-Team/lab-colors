...
- **Дискретные** (переключение темы и изменения атрибутов `style`/`class` в наблюдаемом поддереве) — отслеживаются `MutationObserver` автоматически; layout, canvas и пиксельные изменения он не наблюдает.
