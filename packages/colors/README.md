...
interface WatchController {
  refresh(force?: boolean): ResolvedTheme | null;  // применить, если входная оценка изменилась
...
обеих ролей и неизменных рецептов сборки. Две чистые сборки дополнительно
сравниваются внутри одного pinned Linux job; это проверка повторяемости в данном
job, а не утверждение о cross-run или cross-machine reproducibility. На других
host-платформах checker сообщает только raw/gzip/SHA-диагностику и не выдаёт
локальные байты за канонический release artifact.
