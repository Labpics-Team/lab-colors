...
`adaptTheme` на каждом кадре повторно проверяет уже разрешённые цвета и запускает
новый resolve с переходом только после устойчивого снижения отслеживаемых
метрик.
...
profile, а не как численную гарантию. Полная миграция с порядком отката — в
[руководстве релиза 0.10.0, зафиксированном по хешу](https://github.com/Labpics-Team/lab-colors/blob/52d7895774c2fa3796e51f7133452cf83c09346b/docs/migrations/exact-alpha-glow.md).
...
Режим с dwell-фильтром: каждый кадр повторно проверяет отслеживаемые метрики на
конечном объявленном наборе образцов, а новый resolve и переход запускает только
при устойчивом относительном снижении. Это один порог с выдержкой, не
