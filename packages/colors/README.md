...
Совместимый вход в замороженную историческую гипотезу Dirt Law V1. Значение
лежит в `[0, 1]`, но не является вероятностью и не заявляется как универсальная
психофизическая классификация. Новая контекстная модель публикуется отдельно и
версионированно; старое имя никогда не меняет смысл молча. API `confidence` в
текущем пакете отсутствует.
