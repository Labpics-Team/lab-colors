...
test("WCAG22 WASM role budgets are exact, append-only, and acyclic", async () => {
  ...
  const expectedHashes = {
    ...
    v7: "3d1f474e645880cd485243040a6c5fbe6d0b782e4dce61c01cfe7e18fe9f20b6",
  };
  const documents = {};
  ...
  // V7 (#297 honest-names): правдивый wire-словарь runtime-роли (kebab
  // "text-ratio"/"ui-ratio", ключ floorRatio, честные Display-тексты) стоит
  // ровно +131B над неизменяемым V6 exact-ратчетом — санкционированная
  // версионная правка публичного контракта; compiler-байты не тронуты.
  assert.equal(v7.schemaVersion, 5);
  assert.equal(v7.budgetId, "labcolors-wasm-roles-issue-297-honest-names-v7");
  ...
  assert.deepEqual(v7.buildRecipes.compiler, v6.buildRecipes.compiler);
  assert.equal(v7.buildRecipes.runtime.command, v6.buildRecipes.runtime.command);
  assert.deepEqual(v7.roles.runtime.measurement, {
    issue: 297,
    slice: "GREEN",
    measurementPlatform: "linux-x64",
    rawBytes: 454516,
    sha256: "d5874bb262910672e4c1f7fe57c3532a6b757aa51516e609c50f68cdb8488a69",
  });
  assert.equal(
    v7.roles.runtime.policy.derivation,
    "exact-accepted-issue-297-honest-names-runtime-measurement",
  );
  assert.deepEqual(v7.roles.compiler, v6.roles.compiler);
  assert.equal(v7.roles.runtime.policy.maxRawBytes, v7.roles.runtime.measurement.rawBytes);
  assert.equal(
    v7.roles.runtime.policy.maxRawBytes,
    v6.roles.runtime.policy.maxRawBytes + 131,
    "runtime growth is pinned to exactly the sanctioned honest-names delta",
  );
  assert.ok(v7.roles.runtime.policy.maxRawBytes <= v4.policy.maxRawBytes);
  ...
  assert.match(wholeCallSource, /wasmBudgetPath = resolve\(here, "wasm-size-budget-v6\.json"\)/u);
  ...
  try {
    ...
    const schemaMutations = [
      ...
      ["honest-names ceiling creep", (value) => {
        // Крип сверх санкционированной #297-дельты над неизменяемым V6-ратчетом.
        const ceiling = v6.roles.runtime.policy.maxRawBytes + 131;
...
test("feasibility benchmark keeps V1-V6 history and admits exact V7 Core subjects", () => {
  ...
  assert.deepEqual(contractNames.sort(), [
    ...
    "wcag22-feasibility-benchmark-v7.json",
  ]);
  const immutableArtifactHashes = new Map([
    ...
    ["wcag22-feasibility-benchmark-v6.json", "2e319643e582ca385f988af173b534cd9d1c921f36fcc929f1c56eb8bd978d24"],
  ]);
  ...
  const historicalV6Payload = JSON.parse(read(
    "crates",
    "labcolors-core",
    "contracts",
    "wcag22-feasibility-benchmark-v6.json",
  ...
  const identityProjection = (payload) => ({
    ...
    profileLimits: payload.profileLimits,
    scenarioOrder: payload.scenarioOrder,
    scenarios: payload.scenarios.map((scenario) => ({
      name: scenario.name,
      shape: scenario.shape,
      expected: scenario.expected,
      observedIdentity: scenario.observedIdentity,
    })),
  });
  assert.deepEqual(
    identityProjection(canonicalPayload),
    identityProjection(historicalV6Payload),
    "#297 GREEN rename may change provenance and observations, not the admitted finite algorithm",
  );
  const subjectsByPath = (payload) => new Map(
    payload.subjectManifest.map((subject) => [subject.path, subject.sha256]),
  );
  const historicalSubjects = subjectsByPath(historicalV6Payload);
  const canonicalSubjects = subjectsByPath(canonicalPayload);
  const subjectPaths = [...new Set([
    ...historicalSubjects.keys(),
    ...canonicalSubjects.keys(),
  ])].sort();
  assert.deepEqual(
    subjectPaths.filter(
    ...
    "V7 subject drift must be exactly the admission machinery identity bump " +
      "and the checker temp-dir default promised in #327 review; the #297 " +
      "rename touches solve/semantic/config, which sit in the broad source " +
      "cone (forcing this re-record) but are not admission subjects",
  );
  assert.equal(
    "gitRevision" in canonicalPayload.environment,
    false,
    "durable V7 must not claim an ephemeral measurement commit",
  );
  assert.equal(
    "gitTree" in canonicalPayload.environment,
    false,
    "durable V7 must use its exact source-object cone as the provenance SSOT",
  );
  assert.deepEqual(
  ...
  assert.match(
    ci,
    /v3_artifact="crates\/labcolors-core\/contracts\/wcag22-feasibility-benchmark-v3\.json"[\s\S]*?v3_snapshot=10c44ef0f4248d0390aa339e81c05a6d5e41996f[\s\S]*?--admit-benchmark-binary-sha256 6ac07bad81a204ee8fcee8f94a3c445f881d1ca10edaf4cc4a86a5db0b232e3a[\s\S]*?git worktree add --detach "\$historical_root" "\$v3_snapshot"[\s\S]*?python3 scripts\/check_wcag22_feasibility_benchmark\.py[\s\S]*?--artifact-sha256 46ec939523a9aff4f253c4c74e997dfd95812a694b2507fae885ff60244ade3a[\s\S]*?--self-test/u,
    "V3 must replay through the exact merged Slice-B snapshot",
  );
  assert.match(
    ci,
  ...
  assert.match(
    ci,
    /v6_artifact="crates\/labcolors-core\/contracts\/wcag22-feasibility-benchmark-v6\.json"[\s\S]*?v6_snapshot=3a8d0a6df3c12a3c66363f099c9b12f7d97594f6[\s\S]*?--admit-benchmark-binary-sha256 1e43b4c8418b8f45da3614d9697966874943e034d4275ee2a404f95aa60dbd2a[\s\S]*?git worktree add --detach "\$historical_root" "\$v6_snapshot"[\s\S]*?python3 scripts\/check_wcag22_feasibility_benchmark\.py[\s\S]*?--artifact-sha256 2e319643e582ca385f988af173b534cd9d1c921f36fcc929f1c56eb8bd978d24[\s\S]*?--self-test/u,
    "V6 must replay through the exact merged slice-A snapshot",
  );
  assert.match(
    ci,
  ...
  assert.equal(
    ...
    7,
    "CI must validate exactly six historical and one current benchmark artifact",
  );
  assert.equal(
    ci.match(/git worktree add --detach/gu)?.length,
    7,
    "only the seven main-reachable historical verifier snapshots may use worktrees",
  );
});
