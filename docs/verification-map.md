...
| исторические native-допуски V1/V2 остаются неизменными; V3 заново измеряет текущий neutral-вход через общий finite-domain kernel | V1 checker исполняется в чистом измеренном snapshot `6001cf4`, его applicability verifier — в `94efeee`; V2 побайтно воспроизводится из `4afe61b`; V3 привязан к source SHA `9a0bbc0`, artifact SHA `a5d0fae3253b06eaaed28d396a5118cba4883b1d87a3ff29c8fc2e502261a68c` и полному текущему Core source tree; один applicability-checker разрешает только изменения source-less non-Core workspace-записей | exact Git objects + fail-closed Cargo.lock parser, 13 V3 checker-мутаций и сырые native-наблюдения без timing threshold |
