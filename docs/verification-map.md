...
| исторические native-допуски V1/V2 остаются неизменными; V3 заново измеряет текущий neutral-вход через общий finite-domain kernel | V1 checker исполняется в чистом измеренном snapshot `6001cf4`, его applicability verifier — в `94efeee`; V2 побайтно воспроизводится из стабильного snapshot `4afe61b`; V3 привязан к source SHA `5781d4a`, artifact SHA `ff2ed3c522192fe7c1e1492d59a466dd78c90ba2d5a243474cd4073f93362f53` и полному текущему Core source tree; applicability разрешает только изменения source-less non-Core workspace-записей | exact Git objects + fail-closed Cargo.lock V4 parser, 13 V3 checker-мутаций и сырые native-наблюдения без timing threshold |
