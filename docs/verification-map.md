...
partition `32` байта. Protocol отвергает MAX+1, Core — лишнего кандидата,
локальные exact-MAX oracle — witness с `E=2046`, а Swift-декодер —
терминал без одного ребра. Это доказывает достижимость версионированной
продуктовой границы, но не содержит утверждений о времени, общей памяти
WebAssembly или клиентской задержке: они не замерялись.
