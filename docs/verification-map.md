...
| формула | чем верифицирована | оракул |
|---|---|---|
| `linearise` (порог 0.04045, /12.92, γ 2.4) | `[NEW]` `reference_vectors_deep::wcag_linearise_threshold_is_current_04045` | текущий нормативный текст W3C + IEC sRGB |
...
> Порог `0.03928` из исходного WCAG 2.1 (2018) заменён erratum W3C от
> 2022-02-22 на `0.04045`, совпадающий со стыком IEC EOTF. lab-colors следует
> текущему нормативному значению. Для 8-битных каналов результат не меняется,
> но непрерывные входы между двумя порогами теперь также считаются корректно.
