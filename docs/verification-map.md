...
| исторические native-допуски V1/V2 остаются неизменными; V3 заново измеряет текущий neutral-вход через общий finite-domain kernel | V1 checker исполняется в чистом измеренном snapshot `6001cf4`, его applicability verifier — в `94efeee`; V2 побайтно воспроизводится из `4afe61b`; V3 с artifact SHA `576226821daa3bd0e549cfa50e30785dc71753cc3ab44cae837778b9bb70bb78` проверяет до и после измерения один dependency cone: exact Git objects исходников, SHA-256 verifier/subject-файлов и точный `Cargo.lock`; также фиксируются Rust/Cargo 1.96.0, явный feature set и пустые rustflags | fail-closed strict-JSON parser, точная сверка dependency cone, 56 V3 checker-мутаций и сырые native-наблюдения без timing threshold |
