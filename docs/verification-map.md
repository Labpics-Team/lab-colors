...
нейтральной оси V1 и native-only атомарную explicit-операцию (#296-C2).
Explicit-набор имеет Protocol-проекцию `wcag22-explicit-selection-v1` за
non-default фичей; его WASM/FFI/npm/Swift проекции отсутствуют до #296-C3, и
эти evidence на них не расширяются.
...
| conformance pack 6 добавляет только atomic explicit-selection family (native-only replay); прежние семь family остаются byte-identical, а 15 новых outcomes воспроизводятся canonical protocol encoder побайтно | `pack_v6_contract`; `reference_runner::protocol_reproduces_committed_wcag22_explicit_selection_exactly`; release verifier по восьми семействам | SHA-256 immutable-family guards + дифференциальный protocol replay; npm/Swift реплеят прежние семь |
