...
| исторические native-допуски V1/V2 остаются неизменными; V3 заново измеряет текущий neutral-вход через общий finite-domain kernel | V1 checker исполняется в чистом измеренном snapshot `6001cf4`, его applicability verifier — в `94efeee`; V2 побайтно воспроизводится из `4afe61b`; V3 привязан к source SHA `6c8f66b`, artifact SHA `b7c7e00c644cccc38f9b9a4998975a0eb1321886d71bc72f00a964b5d2ed0982` и полному текущему Core source tree; один applicability-checker разрешает только изменения source-less non-Core workspace-записей | exact Git objects + fail-closed Cargo.lock parser, 13 V3 checker-мутаций и сырые native-наблюдения без timing threshold |
