...
| канонический WASM transport воспроизводим побайтно и завершает все граничные whole-call формы | append-only size V4: `520920 B` + SHA `7e9c3b41d61919e677572094374b9b59f22d61f98c1977b3f5166d96aa684c0e` без headroom и с запретом ослабить V3; V1/V2/V3 неизменны; whole-call V2: 10 форм × 5 fresh-process samples, exact request/outcome hashes и source/pack/toolchain bindings, детерминированная проекция совпадает с V1 | независимые Linux `wc`/`sha256sum` + повторный package-root verifier; latency/maxRSS/pages не превращаются в пороги |
