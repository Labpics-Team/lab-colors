...
Science-vs-fit boundary: the **structural form** (warm gate b=C·sin h, neutral gate sigmoid on C, depth term below cusp-L) is derived from Oklab opponent-colour theory and CAM16-UCS. The **link calibration scalars** (M_W, LESC, W_HUE, CEIL_N_TABLE) are DECLARED-CALIBRATION, never silently tuned. **B0 and BW are cited-measured** (Zone B, 2026-06-29). **Platt scalars CAL_T/CAL_B/CAL_EPS (M-06/M-07/M-08) have been removed** (Zone B slice 3, 2026-06-30): the Platt magnitude link violated the ZERO observer-fit invariant (North). mud = raw_chromatic directly — parameter-free monotone normalization via JND-geometry. CUSP_L_TABLE is cited-and-kept: pure sRGB-gamut geometry, kept as-is per paradigm North.
