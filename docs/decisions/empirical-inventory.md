...
- **GATE 3** — the `// NEEDS-SCIENCE` ↔ provisional-row contract holds both
  ways.
- **GATE 4** — every `// GROUNDED` const's *citation is structurally true*: the
  cited `docs/…\.md` document exists on disk **and** every backtick-quoted
  anchor it cites (e.g. the formula version `` `0.0.98G-4g` ``) is attested
  verbatim in that document. Closes the class "a `GROUNDED` marker cites a
  fabricated, typo'd, or dead-link provenance and still passes" — the blind spot
  GATE 1 (presence only) and GATE 2 (SSOT value/module only) leave open.
...
paper-trail (GATE 1/2/3) and the structural *truth* of a `GROUNDED` marker's
citation (GATE 4: the cited document exists and attests the cited anchor).
Math-vs-paper (R1) — that the magnitude actually equals the cited published
constant — is `golden_tests` (`contrast_core_matches_reference_on_grey_axis`
pins the APCA curve constants against the official `apca-w3` reference);
derivation-identity (R2) is the recompute tests; behavioural value-drift (R3) is
the `sample_hex` / `resolve_set` snapshots. Conflating these into a flat failure
count is a reporting error.
