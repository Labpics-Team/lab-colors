...
> separate deterministic auditor: `C:\Users\Daniel\.agents\tools\mud-oracle\verify_inventory.js`
> (exits 0 on the branch HEAD). The module header in `cleanliness.rs` cross-references each
> constant to its row here.
...
|--------|------|-------|--------|--------|-----------|
| M-01 | `C0` | `0.0395` | `cleanliness.rs` | DERIVED | Grey-frontier chroma floor. In Oklab the sRGB gamut boundary for achromatic greys sits at C ≈ 0.04 (largest C reachable without hue at L=0.5). C0 = 0.0395 places the sigmoid centre ≈ 1 JND below that boundary (JND ≈ 0.012 in Oklab chroma), giving a structurally grounded transition from achromatic to chromatic that is independent of owner clicks. Bound: 0.030 ≤ C0 ≤ 0.050 (±4 JND from gamut grey). |
| M-02 | `JND` | `0.012278` | `cleanliness.rs` | DERIVED | Chroma just-noticeable difference in Oklab. Oklab is perceptually uniform to first order; one JND on the opponent axes a,b is ≈ 0.012 (derived from the ΔE₂₀₀₀ ≈ 1 JND mapped through the Oklab linearisation). JND = 0.01228 is used as the sigmoid transition width for both the neutral gate and the b-gate, encoding that transitions span ≈ 1 perceptual unit. Bound: 0.010 ≤ JND ≤ 0.016. |
...
| M-04 | `B0` | `0.028690` | `cleanliness.rs` | DECLARED-CALIBRATION | Oklab-b warm-gate centre. Structural form (b = C·sin h threshold) is derived from Oklab opponent-colour theory: the yellow–blue axis b separates warm (b > 0) from cool (b < 0). The centre value 0.02869 is Platt-fit on v3 labels; not derivable from theory alone (depends on the empirical boundary of the owner's dirty pocket). |
| M-05 | `BW` | `0.020241` | `cleanliness.rs` | DECLARED-CALIBRATION | Oklab-b warm-gate transition width. Same structural role as JND but for the b-axis warm gate. Value ≈ 1.65 × JND (slightly wider than the neutral gate) reflects that the warm–cool boundary is perceptually softer than the achromatic boundary; fitted on v3. |
...
| M-07 | `CAL_T` | `2.356978` | `cleanliness.rs` | DECLARED-CALIBRATION | Platt link scale (sigmoid temperature on log-raw). Fitted scalar on v3 train split (logistic regression on ln(raw + eps)). No science derivation; pure calibration. |
| M-08 | `CAL_B` | `6.445168` | `cleanliness.rs` | DECLARED-CALIBRATION | Platt link bias (sigmoid intercept on log-raw). Fitted scalar on v3 train split. No science derivation; pure calibration. |
