...
Маршрут выше исполним целиком на `3ad5091e`. Открыты четыре PR; маршрут
меняет один.

**PR 560 — полоса проверяет размер кампании.** У `verification-lanes.yml`
появляется обязательный вход `expect_lanes`, а координатор добавляет в каждую
команду `-f expect_lanes=<число полос плана>`. Полоса первым шагом считает
`16777216 / window_points` и отвергает кампанию, чьё заявленное число полос
этому не равно, — до `checkout`, за секунды вместо часа реплея. Смысл: флаг
`--expect-lanes` защищает намерение оператора там, где устаревший checkout
делает устаревшей и защиту; этот вход держит тот же инвариант там, где код
всегда свежий. После мержа строки шага 3 получают ещё один `-f`; число строк
остаётся 256.
