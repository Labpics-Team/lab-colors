...
## Полоса проверяет координаты кампании

На `main` приём полосы проверяет координаты до `checkout`: вход `expect_lanes`
в `verification-lanes.yml` обязателен (#560, `08b6f265`), полоса отвергает
за секунды вместо часа реплея:
...
Живой диспатч шага 3 передаёт каждой полосе `-f expect_lanes=<число полос
плана>`; число строк остаётся 256.
