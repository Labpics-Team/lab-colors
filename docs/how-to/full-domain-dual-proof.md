...
gh run list --workflow full-domain-run.yml --limit 5 \
  --json databaseId,headSha,event,status,conclusion
```

Идентификатор прогона выбирайте **по совпадению `headSha` с коммитом, который
запускали**, а не по позиции в списке: `--limit 1` берёт верхний, а верхним
может оказаться чужой или параллельный прогон.

Ошибиться дорого, но не фатально: координатор допускает evidence-прогон
только после проверки происхождения — путь workflow, событие `workflow_dispatch`,
`completed`+`success` и `head_sha` — и отказывается до первого диспатча.
Чужой прогон не доедет до полос.
...
```sh
gh workflow run dual-proof.yml -f lane_run_ids=31172034015,31173145286
```
...
Маршрут выше исполним целиком на `3ad5091e`. Открыто шесть PR; маршрут
меняет один.

**PR 560 — полоса проверяет свои координаты.** У `verification-lanes.yml`
появляется обязательный вход `expect_lanes`, а координатор добавляет в каждую
команду `-f expect_lanes=<число полос плана>`. Первым шагом, до `checkout`,
полоса отвергает за секунды вместо часа реплея:

- кампанию, чьё заявленное число полос не равно `16777216 / window_points`;
- `window_start` вне канонической десятичной записи — сравнением сырых байтов,
  потому что и `[ -eq ]`, и `int()` молча принимают окружающие пробельные
  символы, а `$(( ))` читает ведущий ноль как восьмеричную запись;
- `window_start` вне домена или не на шве покрова этой ширины.

Смысл первого правила: флаг `--expect-lanes` защищает намерение оператора там,
где устаревший checkout делает устаревшей и защиту. Смысл остальных: `run-name`
рендерит эти координаты, а сборщик шага 4 связывает прогон с окном плана,
читая имя обратно — старт, который имя не может отрендерить канонически,
даёт полосу, чей час реплея нельзя предъявить никогда.

После мержа строки шага 3 получают ещё один `-f`; число строк остаётся 256.

**PR 559 — содержимое координат компаратора.** Маршрут не трогает, но двигает
пины инвентаря: `ARB_TEST_COUNT_V1` 284 → 292 и оба хеша. Числа в «Граблях»
привязаны к `3ad5091e` и после его мержа устареют — пересчитывайте сниппетом,
не переносите отсюда.

**PR 556 — статическая source-идентичность MPFI.** Маршрут не трогает:
`mpfi/receipt.py` плюс пин MPFI-гейта.

**PR 557 — первое изображение семейства.** Маршрут не трогает: Rust-слой
`crates/labcolors-core`.

**PR 561 — сертификат допускается только к спрошенному региону.** Маршрут не
трогает: Rust-слой. Артефакт может быть цел и при этом быть образом другого
региона; связывание отвергает чужой регион до хеширования payload.

**PR 558 — этот документ.**
